...
def exec_distance_prog(dot, targets, out, names, cg_distance=None,
                       ...
                       target_functions=None, context=False):
    """
    Args:
    ...
    cmd = [prog,
           ...
           "-o", out,
           "-n", names]
    if cg_distance is not None: 
        cmd.extend(["-c", cg_distance])
    if cg_callsites is not None:
        cmd.extend(["-s", cg_callsites])
    if target_functions:
        cmd.extend(["-f", target_functions])
    if context:
        cmd.extend(["-x", "true"])
...
def merge_distance_files(cfg_cg_path, output, pattern="*.distances.txt"):
    with output.open('w') as f:
        for dist in cfg_cg_path.glob(pattern):
...
def calculating_distances(args):
    ...
    bbcalltargets= args.temporary_directory / "BBcalltargets.txt"
    ...
    temp_callgraph_distance = args.temporary_directory / "callgraph.distance.temp.txt"

    merge_distance_files(
            dot_files,
            args.temporary_directory / "callgraph.distance.temp.txt", pattern="*.distances.newcg.txt")
    try:
        cmd = [CG_PY, "-d", callgraph, "-f", ftargets, "-o", callgraph_distance, "-c", temp_callgraph_distance]
        pipe = subprocess.PIPE
        r = subprocess.run(cmd, stdout=pipe, stderr=pipe, check=True)
    except subprocess.CalledProcessError as err:
        with log_p.open("w") as f:
    ...
    next_step(args)
    def calculate_context_from_file(cfg: Path):
        if cfg.stat().st_size == 0: return
        dd_cleanup(cfg)     # for python version
        name = cfg.name.split('.')[-2]
        if name not in callgraph_dot: return
        outname = name + ".context.txt"
        outpath = cfg.parent / outname
        exec_distance_prog(
                cfg,
                bbcalltargets,
                ...
                py_version=args.python_only,
                context=True)
    print(f"({STEP}) Compute context for CFG")
    with ThreadPoolExecutor(max_workers=mp.cpu_count()) as executor:
        results = executor.map(calculate_context_from_file,
                               dot_files.glob("cfg.*.dot"))
    try:
        for r in results: pass  # forward Exceptions
    except subprocess.CalledProcessError as err:
    ...
    merge_distance_files(
            dot_files,
            args.temporary_directory / "BBcontext.txt", pattern="*.context.txt")
...
def main():
    ...
    if not STEP:
        ...
        # after constructing CG
        #\sys here I'd like to produce the related functions
        #next_step(args)

    # here I should also consider the borderline functions
