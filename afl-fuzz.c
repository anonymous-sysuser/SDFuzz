...
static int32_t cur_stack_idx= 0;     /* Stack Index of executed input       */
static int32_t max_stack_idx= 0;     /* Maximal stack idx for any input   */
static int32_t min_stack_idx = 0;     /* Minimal stack idx for any input   */
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  q->stack_idx = cur_stack_idx;
  if (cur_stack_idx > 0) {

    if (max_stack_idx <= 0) {
      max_stack_idx = cur_stack_idx;
      min_stack_idx = cur_stack_idx;
    }
    if (cur_stack_idx > max_stack_idx) max_stack_idx = cur_stack_idx;
    if (cur_stack_idx < min_stack_idx) min_stack_idx = cur_stack_idx;
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  u32* stack_idx1 = (u32*) (trace_bits + MAP_SIZE + 16);
  cur_stack_idx = (int32_t) *stack_idx1;
  ...
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
       ...
       if (top_rated[i]) {
         ...
         // curent q is not favored
         if (q->stack_idx < top_rated[i]->stack_idx)
           continue;
          if(fav_factor > top_rated[i]->exec_us * top_rated[i]->len) continue;
         // if stack idx larger or cost smaller, favor!
...
EXP_ST void setup_shm(void) {
  ...
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE + 20, IPC_CREAT | IPC_EXCL | 0600);
