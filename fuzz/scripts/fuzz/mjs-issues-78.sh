...
echo $'mjs.c:12051,mjs_exec_file\nmjs.c:9781,mjs_exec_internal\nmjs.c:9358,mjs_pop\nmjs.c:9418,mjs_mk_string\nmjs.c:13706,mbuf_resize\n' > $TMP_DIR/BBtargets.txt
$CC -DMJS_MAIN mjs.c $ADDITIONAL -ldl -g -o mjs-bin
cat $TMP_DIR/BBnames.txt | rev | cut -d: -f2- | rev | sort | uniq > $TMP_DIR/BBnames2.txt && mv $TMP_DIR/BBnames2.txt $TMP_DIR/BBnames.txt
cat $TMP_DIR/BBcalls.txt | sort | uniq > $TMP_DIR/BBcalls2.txt && mv $TMP_DIR/BBcalls2.txt $TMP_DIR/BBcalls.txt
$AFLGO/scripts/gen_distance_fast.py $SUBJECT $TMP_DIR mjs-bin
$CC -DMJS_MAIN mjs.c -type=$TMP_DIR/BBcontext.txt -distance=$TMP_DIR/distance.cfg.txt -ldl -g -o mjs-bin
