    ...
    for (vertex_desc n : find_nodes(G, name)) {
        ...
        if (is_cg) {
            for (vertex_desc t : targets) {
                ...
                // there is a path from n to t. we should include n and its callees.
                reachable = 1;
                d += 1.0 / (1.0 + static_cast<double>(shortest));
                ++i;
            }
            if (reachable) {
              // include n and its callees
              bo::graph_traits<graph_t>::vertex_iterator vi, vi_end;
              for (boost::tie(vi, vi_end) = vertices(G); vi != vi_end; ++vi) {
                if(distances[*vi] > 0) {
                  done_functions << G[*vi].name << "\n";
                }
              }
            }
        } else {
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        desc.add_options()
                ...
                ("functions,f", po::value<std::string>(), "Path to output file containing "
                                                      "final concerned functions.")
    ...
    std::ofstream done_function_stream(vm["functions"].as<std::string>());
    if (is_cg) {
        targets = cg_calculation(graph, targets_stream, done_function_stream);
        // TODO \sys compute the functions we care about.
    ...
    for (std::string line; getline(names, line); ) {
        bo::trim(line);
        distance(graph, line, targets, outstream, done_function_stream, bb_distance);
    }
